...
def retarget_naive(seq22):
    """FIX: centrage UNE FOIS (frame 0), pas par frame -- preserve la trajectoire globale."""
    T = seq22.shape[0]
...
def retarget_fk_scale(naive_seq25, edge_length):
    ...
    corrected[:, 0, :] = naive_seq25[:, 0, :]  # trajectoire de la racine preservee (fix propage)
...
CLASSES = [3, 5, 9, 12, 15, 40, 42, 47, 51, 56, 58, 59]
MAX_POOL = 40
print("Calcul des centroides (centrage UNE FOIS, pas par frame)...")
centroids = {}
with torch.no_grad():
    for c in CLASSES:
        mask = y_test == c
        seqs_raw = body1_all[mask][:MAX_POOL]
        embs = []
        for s in seqs_raw:
            valid = np.abs(s).sum(axis=(1, 2)) > 1e-6
            sv = s[valid]
            if sv.shape[0] == 0: continue
            sv_global = sv - sv[0, 0, :]     # FIX: une seule fois, pas par frame
            sv_fair = duplicate_hands(sv_global)
            x = to_shiftgcn_input(sv_fair)
            _, emb = model(x)
            embs.append(emb.squeeze(0).numpy())
        centroids[c] = np.mean(embs, axis=0)
...
print(f"\n{'idx':>3} {'classe':>6} {'nom':<18} {'sim->vraie':>10} {'predite':>7} {'rang':>5} {'OK':>4}")
n_ok = 0
with torch.no_grad():
    for idx, true_cls, name, variant in rows:
        npy_path = f"{RESULTS_DIR}/{idx}_out.npy"
        if not os.path.exists(npy_path) or true_cls not in centroids:
            continue
        ...
        pred_cls = ranked[0][0]
        rank = [c for c, _ in ranked].index(true_cls) + 1
        ok = pred_cls == true_cls
        n_ok += ok
        print(f"{idx:>3} {true_cls:>6} {name:<18} {sims[true_cls]:>10.3f} {pred_cls:>7} {rank:>5} {'OUI' if ok else 'non':>4}")

print(f"\n>>> {n_ok}/{len(rows)} correctement classees (centrage corrige, trajectoire preservee)")
print(f">>> reference precedente (centrage par frame, bug): 1/22")
