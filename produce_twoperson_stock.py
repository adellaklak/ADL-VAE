...
sys.path.insert(0, '/srv/storage/stars@storage3.sophia.grid5000.fr/alakhlef/SK_zsl/wt_motiongpt3-poc')
from combine_two_person import combine_two_person, to_shiftgcn_input_2person, STATIC_OFFSETS, DYNAMIC_CLASSES
...
def process_single(npy_path, edge_length):
    ...
    return duplicate_hands(scaled25)
...
if __name__ == "__main__":
    ...
    RESULTS_DIR = '/srv/storage/stars@storage3.sophia.grid5000.fr/alakhlef/SK_zsl/MotionGPT3/results/motgpt/debug--MoT_vae_trainemb_cls1_cfgrand_2optim_lr2_lr1/samples_2026-08-25-11-38-01'
    MAP_CSV = '/srv/storage/stars@storage3.sophia.grid5000.fr/alakhlef/SK_zsl/MotionGPT3/tmp_prompts/twoperson_v1_k6_map.csv'
    ...
    with open(MAP_CSV) as f:
        for line in f.readlines()[1:]:
            idx, cls, role, seed = line.strip().split(',')
            if int(cls) == -1: continue
            rows.append((int(idx), int(cls), role, seed))

    # regrouper par (classe, role) pour retrouver les paires
    by_class_role = {}
    for idx, cls, role, seed in rows:
        by_class_role.setdefault((cls, role), []).append((idx, seed))

    ROLE_PAIRS = {
        51: [('A_pusher', 'B_pushed')],
        56: [('A_toucher', 'B_touched')],
        58: [('A_walktowards', 'A_walktowards')],  # meme prompt, roles A et B symetriques
        59: [('A_walkapart', 'A_walkapart')],
    }

    with torch.no_grad():
        for cls in [51, 56, 58, 59]:
            roleA_name, roleB_name = ROLE_PAIRS[cls][0]
            listA = by_class_role.get((cls, roleA_name), [])
            listB = by_class_role.get((cls, roleB_name), [])
            print(f"classe {cls}: {len(listA)} sequences role A, {len(listB)} sequences role B")

            embs, meta = [], []
            n_pairs = min(len(listA), len(listB)) if roleA_name != roleB_name else len(listA)

            for i in range(n_pairs):
                idxA, seedA = listA[i]
                if roleA_name == roleB_name:
                    # meme pool: on prend deux echantillons differents comme A et B (walking towards/apart)
                    if i + 1 >= len(listA): break
                    idxB, seedB = listA[i + 1]
                else:
                    idxB, seedB = listB[i]

                pathA = f"{RESULTS_DIR}/{idxA}_out.npy"
                pathB = f"{RESULTS_DIR}/{idxB}_out.npy"
                if not (os.path.exists(pathA) and os.path.exists(pathB)):
                    print(f"  [manquant] idxA={idxA} idxB={idxB}")
                    continue

                seqA = process_single(pathA, edge_length)
                seqB = process_single(pathB, edge_length)
                seqA_c, seqB_c = combine_two_person(seqA, seqB, cls)

                x = to_shiftgcn_input_2person(seqA_c, seqB_c)
                _, emb = model(torch.from_numpy(x))
                embs.append(emb.squeeze(0).numpy())
                meta.append({'idxA': idxA, 'idxB': idxB, 'seedA': seedA, 'seedB': seedB})

            if embs:
                embs_arr = np.array(embs)
                np.save(f"{OUT_DIR}/class_{cls}_embeddings.npy", embs_arr)
                np.save(f"{OUT_DIR}/class_{cls}_qc_mask.npy", np.ones(len(embs), dtype=bool))  # pas de QC 2p pour l'instant
                with open(f"{OUT_DIR}/class_{cls}_meta.json", 'w') as f:
                    json.dump(meta, f, indent=2)
                print(f"  -> {embs_arr.shape[0]} embeddings sauvegardes")
            else:
                print(f"  -> AUCUN embedding produit pour classe {cls}")

    print(f"\n>>> Stock 2-personnes complete dans {OUT_DIR}/")
