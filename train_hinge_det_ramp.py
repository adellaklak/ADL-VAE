...
np.random.seed(seed)
random.seed(seed)
# fix reproductibilite : cuDNN peut choisir des algos de convolution
# non-deterministes meme avec une seed fixee (source du 61.41% -> 53.18%)
...
def train_one_cycle(cycle_num, cycle_length): # 0-10, 1700
    ...
    for epoch in range(s_epoch, e_epoch):
        ...
        # [HINGE-PW] EMA des centroides latents (seen)
        with torch.no_grad():
        ...
        # [HINGE] hinge asymetrique cote texte, sigmoide cote skeleton.
        # --use_pw 1 : ponderation par proximite au centroide latent (outliers attenues)
        _pw = torch.ones(target.shape[0], device=device)
        if args.use_pw == 1:
            with torch.no_grad():
                for _i, _lbl in enumerate(target.cpu().numpy()):
                    _lbl = int(_lbl)
                    if _lbl in latent_centroids and latent_centroids[_lbl].norm() > 0:
                        _d = torch.norm(smu[_i].detach() - latent_centroids[_lbl])
                        _pw[_i] = torch.exp(-_d / args.lmd)
                _pw = _pw / (_pw.sum() + 1e-8) * _pw.shape[0]

        _t_pos = torch.sum(torch.square(t - tfroms), 1)
        _t_neg = torch.sum(torch.square(t - tfroms[t_perm]), 1)
        t_cali = torch.mean(_pw * torch.clamp(_t_pos - _t_neg + args.margin, min=0))
        _s0 = -torch.sum(torch.square(s - sfromt), 1)
        s_cali = torch.mean(_pw * 1.0 / (1.0 + torch.exp(_s0 / args.lmd)))
        ...
        # verification defensive : en --phase val (stage2 du gating), unseen_inds
        # designe un split pseudo-unseen different (rv12_0.npy) ou nos paires
        # diagnostiquees sur le vrai split de test (ru12.npy) n'existent pas.
        for c_unseen, c_seen in HARD_PAIRS_UNSEEN_SEEN:
            if c_unseen in unseen_idx_map and c_seen in latent_centroids and latent_centroids[c_seen].norm() > 0:
        ...
        for c1, c2 in HARD_PAIRS_UNSEEN_UNSEEN:
            if c1 in unseen_idx_map and c2 in unseen_idx_map:
