...
rm -rf "$WD"
echo "===== STRUCTURE ss=12 align_w=$AW (seed=5 hardcodée comme baseline) ====="
python train_structure.py --ntu $ntu --ss $ss --alpha 0.5 --lmd 100 --use_cr_fact 1 --version "struct" \
  ...
  --phase train --mode train --dataset sk_feats/shift_${ss}_r/ --wdir "$WD" | tee structure_12_w${AW}.log
ZSL=$(grep -oP 'increased to \K[0-9.]+' structure_12_w${AW}.log | tail -1)
echo ">>> STRUCTURE ss=12 align_w=$AW : ZSL = $ZSL   (baseline ss=12 = 48.6)"
