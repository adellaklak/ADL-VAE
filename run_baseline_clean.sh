...
python train.py --ntu $ntu --ss $ss --alpha 0.5 --lmd 100 --use_cr_fact 1 --version "base" \
  --st $st --ve $ve --le $le --tm $tm --num_cycles $nc --num_epoch_per_cycle $nepc --latent_size $ls --gpu $gpu \
  --phase train --mode train --dataset sk_feats/shift_${ss}_r/ --wdir results/baseline_5_r | tee baseline_clean.log
echo ">>> BASELINE ss=5 ZSL = $(grep -oP 'increased to \K[0-9.]+' baseline_clean.log | tail -1) (cible 85.3)"
