...
parser.add_argument('--loss_variant', type=str, default='original', help='original|hinge_asym')
parser.add_argument('--margin', type=float, default=2.0, help='marge du hinge texte')
parser.add_argument('--confusion_impl', type=str, default='none', help='none|ema_nograd|sample_grad')
parser.add_argument('--confusion_pairs', type=str, default='manual', help='manual|auto')
parser.add_argument('--confusion_margin', type=float, default=2.5)
parser.add_argument('--confusion_w', type=float, default=1.0)
parser.add_argument('--confusion_topk', type=int, default=4, help='nb de paires si auto')
...
# [CONFUSION] centroides latents des classes seen (EMA)
latent_centroids = {int(c): torch.zeros(latent_size).to(device) for c in seen_inds}
latent_momentum = 0.99
_MANUAL_PAIRS = [(15,16),(13,14)]   # put/take off shoe, put/take off jacket
confusion_pairs = [(a,b) for (a,b) in _MANUAL_PAIRS
                   if a in latent_centroids and b in latent_centroids]
...
def train_one_cycle(cycle_num, cycle_length): # 0-10, 1700
    ...
    for epoch in range(s_epoch, e_epoch):
        ...
        # [CONFUSION] EMA des centroides latents (seen)
        with torch.no_grad():
            for _i, _lbl in enumerate(target.cpu().numpy()):
                _lbl = int(_lbl)
                if _lbl in latent_centroids:
                    latent_centroids[_lbl] = (latent_momentum * latent_centroids[_lbl]
                                              + (1 - latent_momentum) * smu[_i].detach())
        ...
        # [CONFUSION] base loss alternative : hinge asymetrique cote texte
        if args.loss_variant == 'hinge_asym':
            _t_pos = torch.sum(torch.square(t - tfroms), 1)
            _t_neg = torch.sum(torch.square(t - tfroms[t_perm]), 1)
            t_cali = torch.mean(torch.clamp(_t_pos - _t_neg + args.margin, min=0))
        ...
        # [CONFUSION] terme contrastif sur paires confondues
        if args.confusion_impl != 'none' and len(confusion_pairs) > 0:
            _cl = torch.tensor(0.0).to(device); _n = 0

            if args.confusion_impl == 'ema_nograd':
                # reproduction fidele du code d'origine (ATTENTION : gradient nul)
                for ci, cj in confusion_pairs:
                    ci_c, cj_c = latent_centroids[ci], latent_centroids[cj]
                    if ci_c.norm() > 0 and cj_c.norm() > 0:
                        _cl = _cl + torch.clamp(args.confusion_margin - torch.norm(ci_c - cj_c), min=0)
                        _n += 1

            elif args.confusion_impl == 'sample_grad':
                # version differentiable : pousse les smu du batch loin du centroide de la classe confondue
                for ci, cj in confusion_pairs:
                    for a, b in ((ci, cj), (cj, ci)):
                        m = (target == a)
                        if m.sum() > 0 and latent_centroids[b].norm() > 0:
                            d = torch.norm(smu[m] - latent_centroids[b].detach().unsqueeze(0), dim=1)
                            _cl = _cl + torch.clamp(args.confusion_margin - d, min=0).mean()
                            _n += 1

            if _n > 0:
                loss += args.confusion_w * (_cl / _n)
