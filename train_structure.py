...
parser.add_argument('--align_mode', type=str, default='none', help='none|structure')
parser.add_argument('--align_w', type=float, default=1.0, help='poids alignement structure (distance-matching skeleton->texte)')
...
# [STRUCTURE] centroïdes des classes seen dans le latent, mis à jour en EMA
...
def train_one_cycle(cycle_num, cycle_length): # 0-10, 1700
    ...
    for epoch in range(s_epoch, e_epoch):
        ...
        # [STRUCTURE] mise à jour EMA des centroïdes latents (classes seen)
        with torch.no_grad():
        ...
        # [STRUCTURE] alignement structuré : les distances entre prototypes texte
        # doivent imiter les distances entre centroïdes skeleton (dans le latent).
        if args.align_mode == 'structure':
            lc_ids = [c for c in latent_centroids if int(latent_centroids[c].abs().sum()) > 0]
            if len(lc_ids) >= 2:
                C = torch.stack([latent_centroids[c].detach() for c in lc_ids])
                cid2row = {c: i for i, c in enumerate(lc_ids)}
                rows = []; tproto = []
                for c in lc_ids:
                    mask = (target == c)
                    if mask.sum() > 0:
                        rows.append(cid2row[c]); tproto.append(tmu[mask].mean(0))
                if len(rows) >= 2:
                    Tp = torch.stack(tproto); Cs = C[rows]
                    Dt = torch.cdist(Tp, Tp); Ds = torch.cdist(Cs, Cs).detach()
                    Dt = Dt / (Dt.mean() + 1e-6); Ds = Ds / (Ds.mean() + 1e-6)
                    loss += args.align_w * ((Dt - Ds) ** 2).mean()
